...
void print_board(WINDOW *win, int starty, int startx, int height, int width, int square_height, 
	int square_width, int scor);
...
void gameover_print(WINDOW *win, int *running_game, PANEL *game_panel, PANEL *menu_panel, 
	PANEL *bkgr_panel);
...
int main() {
    ...
    while(1) {       
        ...
        print_menu(menu_win, glow);
        if(choice != 0) {
        	// daca exista o selectie, iesi din while
        	break;
        }
    }

    if(choice == 1) {
    	// s-a selectat *New Game*
				...
				case 1:
					...
					switch(key) {
						case KEY_UP: // muta spre nord
							if(valid_move(board, 0, &scor_test, &winner_test, 0)) {
								// e valida mutarea?	
...
// Printeaza fereastra de joc
void print_board(WINDOW *win, int starty, int startx, int height, int width, 
	int square_height, int square_width, int scor) {	
	...
	wattron(win, A_BOLD);
	for(i = startx; i <= endx; i = i + square_width) {
		// printarea liniilor verticale
		for(j = starty; j <= endy; j++) {
			mvwaddch(win, j, i, ACS_VLINE);
		}
	}

	for(i = starty; i <= endy; i = i + square_height) {
		// printarea liniilor orizontale
	...
	for(i = startx + square_width; i <= endx - square_width; i = i + square_width) {
		// pentru top si bottom	
		mvwaddch(win, starty, i, ACS_TTEE);
		mvwaddch(win, endy, i, ACS_BTEE);
	}
	for(i = starty + square_height; i <= endy - square_height; i = i + square_height) {
		// pentru left si right	
		mvwaddch(win, i, startx, ACS_LTEE);
		mvwaddch(win, i, endx, ACS_RTEE);	
		for(j = startx + square_width; j <= endx - square_width; j = j + square_width) {
			// pentru celulele din interiorul tabelei
...
void init_board(int board[4][4]) {
	...
	while(1) {
		// pana cand gasesti 2 elemente potrivite
...
// Printeaza matricea si mesajul de win
void fill_board(WINDOW *win, int board[4][4], int winner, int scor) {
	int i,j, nline, ncols;
	int startx, starty;
	...
	if(winner) {
		// afiseaza mesajul in cazul in care castiga
	...
	for(i = 0; i < 4; i++) {
		for(j = 0; j < 4; j++) {
			if(board[i][j] != 0) {
				// printarea colorata a elementelor matricei
				switch(board[i][j]) {
					case 2: wattron(win, COLOR_PAIR(1));
							break;
					case 4: wattron(win, COLOR_PAIR(2));
							break;
					case 8: wattron(win, COLOR_PAIR(4));
							break;
					case 16: wattron(win, COLOR_PAIR(5));
							break;
					case 32: wattron(win, COLOR_PAIR(6));
							break;
					case 64: wattron(win, COLOR_PAIR(7));
							break;
					case 128: wattron(win, COLOR_PAIR(8));
							break;
					case 256: wattron(win, COLOR_PAIR(9));
							break;
					case 512: wattron(win, COLOR_PAIR(10));
							break;
					case 1024: wattron(win, COLOR_PAIR(11));
							break;
					case 2048: wattron(win, COLOR_PAIR(12));
							break;
					case 4096: wattron(win, COLOR_PAIR(13));
							break;
					default: wattron(win, COLOR_PAIR(3));
				}
	...
	if(dir == 0) {
		...
		for(j = 0; j < 4; j++) {
			for(i = 0; i < 4; i++) {
				...
				if(board[i][j] != 0) {
					if(i > 0) {
						// exista campuri valide vecine in directia selectata?
						if(board[i - 1][j] == board[i][j]) {
							// daca da, se pot combina?
...
int valid_move(int board[4][4], int dir, int *scor, int *winner, int mode) {
	...
	for(i = 0; i < 4; i++) {
		// copierea matricei intr-o matrice test
		for(j = 0; j < 4; j++) {
			test[i][j] = board[i][j];
		}
	}
	// aplicarea mutarii pe matricea de test
	game(test, dir, scor, winner); 
	for(i = 0; i < 4; i++) {
		for(j = 0; j < 4; j++) {
...
// Verifica daca jocul e terminat (nu mai exista mutari valide)
int gameover(int board[4][4]) {
	int test1 = 0, test2 = 0, i, j, dir0, dir1, dir2, dir3;
	for(i = 0; i < 4; i++) {
		// verificare existenta celule libere
		for(j = 0; j < 4; j++) {
			if(board[i][j] == 0) {
				return 0; 
			}
	...
	if(dir0 || dir1 || dir2 || dir3) {
		...
		return 1; // altfel, gameover
	}
}

// Printeaza mesajul de gameover si revine la meniu
void gameover_print(WINDOW *win, int *running_game, PANEL *game_panel, 
	PANEL *menu_panel, PANEL *bkgr_panel) {
...
int automove(WINDOW *win, int board[4][4], int *scor, int *winner) {
	...
	if(best_dir >= 0) {
		// daca exista inca miscari valide, execut-o pe cea mai eficienta
		...
		return 0; // altfel, nu mai exista miscari valide
	}
}

// Executa back-up si undo
...
void backup_undo(int board[4][4], int *scor, int *winner, int undo[4][4], 
	int *scor_backup, int *winner_backup, int mode) {
